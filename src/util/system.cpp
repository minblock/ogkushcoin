...
const char * const BITCOIN_CONF_FILENAME = "ogkushcoin.conf";
...
#else
    const char* pszModule = "ogkushcoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "OGKushcoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/OGKushcoin";
#else
    // Unix-like
    return pathRet / ".ogkushcoin";
#endif
#endif
}
