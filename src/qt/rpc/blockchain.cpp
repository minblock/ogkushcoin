...
UniValue gettxout(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 3)
        throw std::runtime_error(
            ...
            "     \"addresses\" : [          (array of string) array of ogkush addresses\n"
...
static UniValue getchaintips(const JSONRPCRequest& request)
{
     ...
     * Algorithm:
     *  - Make one pass through mapBlockIndex, ogkushking out the orphan blocks, and also storing a set of the orphan block's pprev pointers.
