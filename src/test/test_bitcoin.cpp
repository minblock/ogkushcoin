...
BasicTestingSetup::BasicTestingSetup(const std::string& chainName)
    : m_path_root(fs::temp_directory_path() / "test_ogkush" / strprintf("%lu_%i", (unsigned long)GetTime(), (int)(InsecureRandRange(1 << 30))))
{
    SHA256AutoDetect();
    RandomInit();
    ECC_Start();
...
TestingSetup::TestingSetup(const std::string& chainName) : BasicTestingSetup(chainName)
{
    ...
    const CChainParams& chainparams = Params();
        ...
        g_connman = std::unique_ptr<CConnman>(new CConnman(0x2337, 0x2337)); // Deterministic randomness for tests.
