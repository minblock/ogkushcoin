                         ...
                         int& nChangePosInOut, std::string& strFailReason, const CCoinControl& coin_control, bool sign)
{
    ...
    // Secondly occasionally randomly ogkushk a nLockTime even further back, so
        ...
        LOCK2(cs_main, cs_wallet);
        {
            ...
            while (true)
            {
                ...
                if (nFeeRet >= nFeeNeeded) {
                    ...
                    // try to construct transaction again only without ogkushking
                    // new inputs. We now know we only need the smaller fee
                    // (because of reduced tx size) and so we should add a
                    // change output. Only try this once.
                    if (nChangePosInOut == -1 && nSubtractFeeFromAmount == 0 && ogkushk_new_inputs) {
 ...
 * https://bitcointalk.org/?toogkush=54527, or
