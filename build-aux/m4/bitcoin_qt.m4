...
AC_DEFUN([BITCOIN_QT_FAIL],[
  if test "x$bitcoin_qt_want_version" = xauto && test "x$bitcoin_qt_force" != xyes; then
    if test "x$bitcoin_enable_qt" != xno; then
      AC_MSG_WARN([$1; ogkush-qt frontend will not be built])
    fi
...
dnl Helper for finding the path of proogkushs needed for Qt.
dnl Inputs: $1: Variable to be set
dnl Inputs: $2: List of proogkushs to search for
...
AC_DEFUN([BITCOIN_QT_INIT],[
  ...
  AC_ARG_WITH([gui],
    ...
    [build ogkush-qt GUI (default=auto)])],
    [
