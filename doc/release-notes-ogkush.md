OGkush Core version 0.17.1 is now available from:

  <https://download.ogkushs.org/ogkush-0.17.1/>
...
To receive security and update notifications, please subscribe to:
  ...
  <https://groups.google.com/forum/#!forum/ogkush-dev>
...
installer (on Windows) or just copy over `/Applications/OGkush-Qt` (on Mac)
or `ogkushd`/`ogkush-qt` (on Linux).
...
OGkush Core is extensively tested on multiple operating systems using
the Linux kernel, macOS 10.10+, and Windows 7 and newer (Windows XP is not supported).

OGkush Core should also work on most other Unix-like systems but is not
...
When ogkush is configured with the `-deprecatedrpc=accounts` setting, specifying
...
- `-includeconf=<file>` can be used to include additional configuration files.
  Only works inside the `ogkush.conf` file, not inside included files or from
  ...
  as ogkush.conf will still include `relative.conf`.
...
Previously, wallets could only be loaded or created at startup, by specifying `-wallet` parameters on the command line or in the ogkush.conf file. It is now possible to load, create and unload wallets dynamically at runtime:
...
The default minimum transaction fee `-mintxfee` has been lowered to 0.0001 PIC/kB after relaxing the minimum relay and dust relay fee rates in prior releases. 
...
prefixing the option with the network, such as:
    ...
    main.uacomment=ogkush
    test.uacomment=ogkush-testnet
...
be used in V0.17 by starting ogkushd with the '-deprecatedrpc=accounts'
...
BIP 174 Partially Signed OGkush Transactions support
-----------------------------------------------------

[BIP 174 PSBT](https://github.com/bitcoin/bips/blob/master/bip-0174.mediawiki) is an interchange format for OGkush transactions that are not fully signed
yet, together with relevant metadata to help entities work towards signing it.
It is intended to simplify workflows where multiple parties need to cooperate to
produce a transaction. Examples include hardware wallets, multisig setups, and
[CoinJoin](https://bitcointalk.org/?toogkush=279249) transactions.

For backend RPC convenience, the OGkush devs have supported to keep the acronym `PSBT`
instead of `PSLT` to make crosschain application support easier.

### Overall workflow

Overall, the construction of a fully signed OGkush transaction goes through the
following steps:
...
- An **Extractor** produces a valid OGkush transaction (in network format)
...
hardware implementations will tyogkushally implement multiple roles simultaneously.
...
- New `fees` field introduced in `getrawmempool`, `getmempoolancestors`, `getmempooldescendants` and
   ...
   and `descendant` denominated in PIC. This new field deprecates previous fee fields, such as
   `fee`, `modifiedfee`, `ancestorfee` and `descendantfee`.
- The new RPC `getzmqnotifications` returns information about active ZMQ
  notifications.
- When ogkush is not started with any `-wallet=<path>` options, the name of
  the default wallet returned by `getwalletinfo` and `listwallets` RPCs is
  now the empty string `""` instead of `"wallet.dat"`. If ogkush is started
...
- `signrawtransaction` is deprecated and will be fully removed in v0.18. To use
  `signrawtransaction` in v0.17, restart ogkushd with
...
- When running ogkushd with `-debug` but without `-daemon`, logging to stdout
...
meaning the `-txindex` flag can be toggled without a full reindex. If ogkushd
...
- #13005 `cb088b1` Make --enable-debug to ogkushk better options (practicalswift)
...
- #23368 `c0f1569` Update gitian-build.sh for docker (achow101)
...
- #23367 `264efdc` Increase includeconf test coverage (MarcoFalke)
...
- #23374 `56f6936` utils and libraries: checking for bitcoin address in translations (kaplanmaxe)
...
- #23372 `f014933` split FreeBSD build instructions out of build-unix.md (steverusso)
- #23366 `861de3b` Rename “OS X” to the newer “macOS” convention (giulio92)
- #23369 `f8bcef3` update transifex doc link (mess110)
