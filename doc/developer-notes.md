...
- [Development guidelines](#development-guidelines)
    - [General OGKushcoin Core](#general-ogkushcoin-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
OGKushcoin Core uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
Run with the `-testnet` option to run with "play ogkushcoins" on the test network, if you
...
OGKushcoin Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/minblock/ogkushcoin/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_ogkushcoin
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_ogkushcoin --log_level=test_suite
$ valgrind -v --leak-check=full src/ogkushcoind -printtoconsole
...
# A coverage report will now be accessible at `./test_ogkushcoin.coverage/index.html`.
```
...
To profile a running ogkushcoind process for 60 seconds, you could use an
invocation of `perf record` like this:

```sh
$ perf record \
    -g --call-graph dwarf --per-thread -F 140 \
    -p `pgrep ogkushcoind` -- sleep 60
```
...
OGKushcoin Core can be compiled with various "sanitizers" enabled, which add
...
However, in open source software such as OGKushcoin Core, where everyone uses
...
pay attention to for reviewers of OGKushcoin Core code.

General OGKushcoin Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers.
  ...
  - *Rationale*: OGKushcoin Core uses tinyformat, which is type safe. Leave them out to avoid confusion.
...
Some of these are maintained by active developers of OGKushcoin Core, in which case changes should probably go
directly upstream without being PRed directly against the project. They will be merged back in the next
subtree merge.

Others are external projects without a tight relationship with our project. Changes to these should also
be sent upstream, but bugfixes may also be prudent to PR against OGKushcoin Core so that they can be integrated
...
file descriptors, it will cause problems with OGKushcoin's `select()` loop, because
...
compatibility between nodes. This happened in OGKushcoin 0.8 (when LevelDB was
...
would be to revert the upstream fix before applying the updates to OGKushcoin's
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `ogkushcoin-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `ogkushcoin-cli` and the GUI debug console use this table to determine how to
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of OGKushcoin Core, the wallet was always
