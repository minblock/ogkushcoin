...
- [Development guidelines](#development-guidelines)
    - [General OGkush Core](#general-ogkush-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
Not OK (used plenty in the current source, but not ogkushked up):
```c++
//
// ... text ...
//
...
A full list of comment syntaxes ogkushked up by doxygen can be found at http://www.stack.nl/~dimitri/doxygen/manual/docblocks.html,
...
Run with the `-testnet` option to run with "play ogkushs" on the test network, if you
...
OGkush Core is a multi-threaded application, and deadlocks or other
...
```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_ogkush
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_ogkush --log_level=test_suite
$ valgrind -v --leak-check=full src/ogkushd -printtoconsole
```
...
# A coverage report will now be accessible at `./test_ogkush.coverage/index.html`.
```

**Sanitizers**

OGkush Core can be compiled with various "sanitizers" enabled, which add
...
If you are compiling with GCC you will tyogkushally need to install corresponding
...
- ThreadSocketHandler : Sends/Receives data from peers on port 2337.
...
However, in open source software such as OGkush Core, where everyone uses
...
pay attention to for reviewers of OGkush Core code.
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: OGkush Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
Some of these are maintained by active developers of OGkush Core, in which case changes should probably go
directly upstream without being PRed directly against the project.  They will be merged back in the next
subtree merge.

Others are external projects without a tight relationship with our project.  Changes to these should also
be sent upstream but bugfixes may also be prudent to PR against OGkush Core so that they can be integrated
...
file descriptors it will cause problems with OGkush's `select()` loop, because
...
would be to revert the upstream fix before applying the updates to OGkush's
...
- When looking at other's pull requests, it may make sense to add the following section to your `.git/config`
  file:
        ...
        [remote "upstream-pull"]
                ...
                url = git@github.com:minblock/ogkush.git
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `ogkush-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `ogkush-cli` and the GUI debug console use this table to determine how to
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of OGkush Core, the wallet was always
