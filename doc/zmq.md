...
The OGKushcoin Core daemon can be configured to act as a trusted "border
router", implementing the ogkushcoin wire protocol and relay, making
...
The ZeroMQ feature in OGKushcoin Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building ogkushcoind:
...
For instance:
    ...
    $ ogkushcoind -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubhashtx=tcp://192.168.1.2:28332 \
               -zmqpubrawtx=ipc:///tmp/ogkushcoind.tx.raw \
...
These options can also be provided in ogkushcoin.conf.
...
From the perspective of ogkushcoind, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into ogkushcoind directly. Furthermore, no information is
...
using. OGKushcoind appends an up-counting sequence number to each
