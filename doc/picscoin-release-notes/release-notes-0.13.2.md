OG Core version 0.13.2 is now available from:
  ...
  <https://download.og.bitwall.ca/OGC-0.13.2.1/>
...
Please report bugs using the issue tracker at github:
  ...
  <https://github.com/minblock/OGC/issues>
...
updates will be released anymore. Without security updates, using a OGC
...
This allows running OGCd without having to do any manual configuration.
...
`OGC.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
Then, to tunnel a SSL connection on 21338 to a RPC server bound on localhost on port 19334 do:

    stunnel -d 21338 -r 127.0.0.1:19334 -p stunnel.pem -P ''
...
caching. A sample config for apache2 could look like:
    ...
    <Location /OGCrpc>
        ...
        # optional bypass OGCd rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< OGCrpc:<password>
    </Location>
...
Restarting OGCd will no longer clear out the list of banned peers; instead
...
- Adding `dbcache=100` in OGC.conf
...
For example:
    ...
    $ src/OGC-cli -stdin walletpassphrase
    mysecretcode
    120
...
- `OGC-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `OGC-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of OGCoins they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of OGCoins being spent is exactly
...
- `OGC-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `OGC-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
