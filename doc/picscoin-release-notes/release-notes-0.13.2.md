OGKush Core version 0.13.2 is now available from:

  <https://download.ogkushs.org/ogkush-0.13.2.1/>
...
updates will be released anymore. Without security updates, using a ogkush
...
ECDSA signatures inside OGKush transactions now use validation using
...
Previous versions of OGKush Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of OGKush Core was to raise the default minimum
relay fee.

OGKush Core 0.13.2 will have a strict maximum size on the mempool. The
...
OGKush Core 0.13.2 also introduces new default policy limits on the length and
...
This allows running ogkushd without having to do any manual configuration.
...
OGKush Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. OGKush Core will relay transactions with
...
entirely in OGKush Core 0.13, and it is at this time undecided whether the
...
OGKush Core automatically creates a hidden service to listen on, without
manual configuration. OGKush Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if OGKush Core is listening, and
...
OGKushd can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals OGKush
Core to use floating fees. By default, OGKush Core will use floating
fees.
...
(default: `0.0002` PIC/kB).

At all times, OGKush Core will cap fees at `-maxtxfee=<x>` (default:
0.10) PIC.
Furthermore, OGKush Core will never create transactions paying less than
...
`ogkush.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
caching. A sample config for apache2 could look like:
    ...
    <Location /ogkushrpc>
        ...
        # optional bypass ogkushd rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< ogkushrpc:<password>
    </Location>
...
Restarting ogkushd will no longer clear out the list of banned peers; instead
...
- Adding `dbcache=100` in ogkush.conf
...
For example:
    ...
    $ src/ogkush-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The OGKush Core code base has
...
- `ogkush-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `ogkush-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
for a new opcode in the OGKush scripting system that in combination with
...
OGKush transactions currently may specify a locktime indicating when
...
HD wallets are incompatible with older versions of OGKush Core.
...
The two phases are distinct in the OGKush-Qt GUI. During the first one,
...
- **Weighting data based on how it affects node performance:** Some parts of
  each OGKush block need to be stored by nodes in order to validate future
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of ogkushs they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of ogkushs being spent is exactly
...
- **Increased security for multisig:** OGKush addresses (both P2PKH addresses
  ...
  believed to be provided by OGKush's choice of parameters for its Elliptic
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  produce an accurate copy of OGKush's current ledger (called the UTXO set),
  ...
  note that OGKush Core does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  OGKush users to individually opt-in to almost any change in the OGKush
  Script language when those users receive new transactions.  Features
  currently being researched by Bitcoin and OGKush Core contributors that may
...
segwit's start date of 1 January 2017 miners can update the OGKush
client to OGKush Core 0.13.2 to signal for segwit support. When a
...
Since OGKush Core 0.10.0, nodes have defaulted to only relaying and
...
- `ogkush-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `ogkush-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
